...
def load_static_file(filename: str) -> Optional[bytes]:
    """
    Load any static file from the static directory.
    """
    file_path: Path = (STATIC_DIR / filename).resolve()
    if not file_path.is_relative_to(STATIC_DIR.resolve()):
        return None
...
async def handle_client(
    ...
    if path.endswith(".css"):
        css_filename = path.lstrip("/")  # Remove leading slash
        css_content = load_static_file(css_filename)
        if css_content:
